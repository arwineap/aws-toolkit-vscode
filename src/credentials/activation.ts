...
import { toastOnConnect } from '../shared/extensionUtilities'
...
export async function loginWithMostRecentCredentials(
    ...
    if (profiles && profileNames.length === 1 && profileNames[0] === 'profile:default') {
        // Auto-connect if there is exactly one profile, named "default".
        let id = await loginManager.login(profiles['profile:default'])
        toastOnConnect(id)
